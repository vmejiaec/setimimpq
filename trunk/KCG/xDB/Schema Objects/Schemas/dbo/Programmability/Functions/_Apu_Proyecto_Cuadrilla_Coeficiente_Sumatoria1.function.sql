CREATE FUNCTION [dbo].[_Apu_Proyecto_Cuadrilla_Coeficiente_Sumatoria1]
(
...
BEGIN
    ...
    select @v_total_categoria =
      (select
             sum(dbo._Apu_Proyecto_Cuadrilla_Coeficiente_Categoria_1(@p_apu_proyecto_id, apu_categoria_id)) 
    from
   ...
   select @v_total_mano_obra=
     ( select
           sum(dbo._Apu_Proyecto_Cuadrilla_Coeficiente_ManoObra_1(@p_apu_proyecto_id, apu_mano_obra_id))
       from 
