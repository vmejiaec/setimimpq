CREATE FUNCTION [dbo].[_Apu_Oferta_Cuadrilla_Coeficiente_Sumatoria]
(
...
BEGIN
    ...
    select @v_total_categoria =
      (select
             sum(dbo._Apu_Oferta_Cuadrilla_Coeficiente_Categoria(@p_apu_Oferta_id, apu_categoria_id)) 
    from
   ...
   select @v_total_mano_obra=
     ( select
           sum(dbo._Apu_Oferta_Cuadrilla_Coeficiente_Mano_Obra(@p_apu_Oferta_id, apu_mano_obra_id))
       from 
