CREATE PROCEDURE dbo.Com_Contrato_SelByPla_Doc_Codigo
(
	@p_Pla_Doc_Codigo varchar(200)
)
AS
--	SET NOCOUNT ON;
SELECT        
	...
	(select tar.Codigo from Pla_Tarea tar where tar.Id =
		(select doc.Pla_Tarea_Id from Pla_Doc doc where doc.Id = Pla_Doc_Id)) Pla_Tarea_Codigo,
	(select tar.Nombre from Pla_Tarea tar where tar.Id =
		(select doc.Pla_Tarea_Id from Pla_Doc doc where doc.Id = Pla_Doc_Id)) Pla_Tarea_Nombre,
  ...
  Fecha_Juridico
FROM	Com_Contrato
where (select doc.Codigo from Pla_Doc doc where doc.Id = Pla_Doc_Id) = @p_Pla_Doc_Codigo
