...
CREATE PROCEDURE [dbo].[Pla_Poa_SelByAnio]
( @p_Anio varchar(4) )
AS
	SET NOCOUNT ON;
SELECT        
...
where 
	Pla_Tarea_Id in
		(select t.Id from Pla_Tarea t where t.Pla_Cta_Id in
			(Select c.Id from Pla_Cta c where c.Anio = @p_Anio)
		)
order by Pla_Partida_Codigo