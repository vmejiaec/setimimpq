CREATE PROCEDURE dbo.Com_Contrato_SelByLikePla_Tarea_Nombre
(
  @p_Pla_Tarea_Nombre varchar(200)
)
AS
--	SET NOCOUNT ON;
SELECT        
  ...
  Fecha_Juridico
FROM	Com_Contrato
where (select UPPER(tar.Nombre) from Pla_Tarea tar where tar.Id =
		(select doc.Pla_Tarea_Id from Pla_Doc doc where doc.Id = Pla_Doc_Id)
	  ) like '%' + UPPER(@p_Pla_Tarea_Nombre) + '%'
order by Pla_Tarea_Nombre
