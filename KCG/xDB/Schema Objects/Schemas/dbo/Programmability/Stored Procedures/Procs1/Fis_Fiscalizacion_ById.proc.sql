-- El calculo de FEchaIndice_Base, es:
---Si el tipo del contrato es PRINCIPAL, la fecha base es igual a la 
-- fecha de cierre de ofertas + 30 día calendario.
--Si el tipo de contrato es COMPLEMENTARIO, la fecha base es igual a la fecha de aprobación del APU.
CREATE PROCEDURE dbo.Fis_Fiscalizacion_ById
(
   @p_Ver_Version_Id varchar (17),
   @p_Id varchar(17),
   @p_Jefe_Fiscalizacion varchar(17) -- Per_Personal_Id (viene del scope)
)
AS
	SET NOCOUNT ON;
BEGIN
...
--    IF (@v_Fecha_Indice_Base IS NULL)
--BEGIN
--  Select @v_Publicacion_Base = 'nulo'
--END
--    ELSE
--BEGIN
--  Select @v_Publicacion_Base = 'SIII'
--END
	...
	Set @v_Plazo_Ampliacion=@v_Plazo_Prorroga;
		...
		SELECT      
			...
			dbo.Per_Personal_Jefe_Fiscalizacion( @p_jefe_fiscalizacion) Jefe_Fiscalizacion,
			dbo.Dominio('PER_PERSONAL','JEFE_FISCALIZACION', dbo.Per_Personal_Jefe_Fiscalizacion(@p_jefe_fiscalizacion) , @p_Ver_Version_Id) Jefe_Fiscalizacion_Nombre,
