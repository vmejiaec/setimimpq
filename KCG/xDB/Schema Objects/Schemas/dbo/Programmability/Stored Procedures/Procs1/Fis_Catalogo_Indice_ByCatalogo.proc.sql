...
CREATE PROCEDURE [dbo].[Fis_Catalogo_Indice_ByCatalogo]
(
	-- Parámetros Obligatorios
	@p_Ver_Version_Id varchar(17),
	@p_Fis_Catalogo_Id varchar(17),
	-- Parámetros Opcionales
	@p_Apu_Indice_Codigo_Desde varchar(200),
	@p_Apu_Indice_Codigo_Hasta varchar(200),
...
(
	SELECT
			...
			Apu_Indice_Id, 
			Valor,
			ROW_NUMBER() OVER (ORDER BY dbo._Apu_Indice_Nombre(Apu_Indice_Id)) AS RowNumber
	FROM Fis_Catalogo_Indice
	WHERE 
			Fis_Catalogo_Id = @p_Fis_Catalogo_Id
	AND	Convert(numeric,dbo._Apu_Indice_Codigo(Apu_Indice_Id)) between Convert(numeric,	@p_Apu_Indice_Codigo_Desde) and Convert(numeric,@p_Apu_Indice_Codigo_Hasta) 
	AND dbo._Apu_Indice_Nombre(Apu_Indice_Id) like @p_Apu_Indice_Nombre
...
order by dbo._Apu_Indice_Nombre(Apu_Indice_Id)
